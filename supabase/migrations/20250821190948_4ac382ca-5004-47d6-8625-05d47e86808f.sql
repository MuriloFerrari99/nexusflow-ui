...
-- Tabela de notificações de projeto
...
-- Políticas RLS para project_notifications
CREATE POLICY "Users can view their notifications" ON public.project_notifications
FOR SELECT USING (
    company_id = get_user_company_id() AND 
    (recipient_id = auth.uid() OR is_company_admin())
);

CREATE POLICY "System can create notifications" ON public.project_notifications
