...
CREATE TABLE public.suppliers (
    ...
    lead_time_default INTEGER DEFAULT 7,
...
CREATE POLICY "Users can manage supplier contacts" ON public.supplier_contacts
FOR ALL USING (EXISTS (SELECT 1 FROM public.suppliers WHERE id = supplier_contacts.supplier_id AND company_id = get_user_company_id()));

-- Create RLS policies for supplier_price_lists
CREATE POLICY "Users can manage supplier price lists" ON public.supplier_price_lists
FOR ALL USING (EXISTS (SELECT 1 FROM public.suppliers WHERE id = supplier_price_lists.supplier_id AND company_id = get_user_company_id()));

-- Create RLS policies for purchase_requisitions
CREATE POLICY "Users can manage company purchase requisitions" ON public.purchase_requisitions
FOR ALL USING (company_id = get_user_company_id());

-- Create RLS policies for purchase_requisition_items
...
-- Create RLS policies for rfq_suppliers
...
-- Create RLS policies for rfq_items
...
-- Create RLS policies for rfq_quotes
...
-- Create RLS policies for landed_cost_allocations
