...
import { ArrowUp, Search, Clock, CheckCircle, AlertTriangle, Calendar, Filter } from "lucide-react";
...
export function ContasReceber({ refresh }: ContasReceberProps) {
    ...
    try {
      ...
      if (statusFilter && statusFilter !== "all") {
        if (statusFilter === "overdue") {
          ...
          query = query.eq('status', 'pending').lte('due_date', nextWeek.toISOString().split('T')[0]);
        } else {
      ...
      if (searchTerm) {
        filteredData = filteredData.filter(conta =>
          ...
          conta.invoice_number?.toLowerCase().includes(searchTerm.toLowerCase())
        );
      }
      ...
      const contasWithOverdue = filteredData.map(conta => {
        ...
        return {
          id: conta.id,
          description: `Fatura #${conta.invoice_number}`,
          amount: conta.net_amount,
      ...
      console.error('Error loading contas a receber:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getKPIs = () => {
    ...
    const received = contas.filter(conta => conta.status === 'paid')
      .reduce((sum, conta) => sum + conta.amount, 0);
    ...
    return { total, overdue, received, dueSoon };
  };
  ...
  const getStatusBadge = (status: string, daysOverdue?: number) => {
    const statusConfig = {
      pending: { label: 'A Vencer', variant: 'default' as const, color: 'bg-blue-100 text-blue-800' },
      overdue: { label: `Vencida (${daysOverdue}d)`, variant: 'destructive' as const, color: 'bg-red-100 text-red-800' },
      paid: { label: 'Recebida', variant: 'secondary' as const, color: 'bg-green-100 text-green-800' },
                ...
                {contas.length === 0 ? (
                  ...
                  contas.map((conta) => (
                    <TableRow key={conta.id} className={conta.status === 'overdue' ? 'bg-red-50' : ''}>
                      ...
                      <TableCell>{conta.payment_method}</TableCell>
                      <TableCell>
